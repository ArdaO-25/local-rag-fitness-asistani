"""
rag.py — Yerel RAG sisteminin paylasilan mantigi (retrieval, prompt
olusturma, cevap temizligi, model yukleme).

Bu modul hem app.py (komut satiri arayuzu) hem arayuz.py (Streamlit
arayuzu) tarafindan ortak kullanilir. Iki arayuz de burada tanimlanan
ayni fonksiyonlari cagirir; RAG mantiginin kendisi sadece burada, tek
bir yerde tanimlidir.
...
Cevabin EN FAZLA 3-4 cumle olsun. Sadece sorulan soruyu dogrudan yanitla;
ek varyasyonlari, ipuclarini, genel antrenman tavsiyelerini veya BAGLAM'da
olup da soruyla dogrudan ilgili olmayan detaylari atla. Kisa cevap uzun
cevaptan HER ZAMAN tercih edilir.
...
BAGLAM'da gordugun dosya adlarini kullan. Unutma: cevabin EN FAZLA 3-4
cumle olmali.
...
def is_program_request(query):
    ...
    gosteririz (answer_query'de SIMILARITY_THRESHOLD kontrolüyle ayni
    sekilde). Bunun nedeni: SYSTEM_PROMPT'a "boyle bir istegi reddet" diye
    bir kural eklemistik, ama model reddederken bile kendi cumlelerini
    kurarken bozuk/tekrarli metin uretebiliyordu (orn. "Baglam'da 4 günlük
    antren" diye yarida kesilen bir cumle). Niyeti sorunun kendisinden --
    modele hic sormadan -- tespit etmek, tutarli ve her zaman ayni (sabit)
...
class ModelBundle:
    """Yuklu embedding ve sohbet modellerini, hazir istemcileriyle
    birlikte tutan basit bir kap. load_models() tarafindan dondurulur.

    load_seconds: load_models()'in tamamini calistirmanin ne kadar
    surdugu (FoundryLocalManager.initialize + iki modelin download+load'i
    dahil). Bu deger BIR KEZ olculur; ModelBundle onbelleklendiginde
    (orn. arayuz.py'de @st.cache_resource ile) sonraki her kullanimda
    da ayni (ilk yuklemedeki gercek) sureyi tasimaya devam eder --
    boylece arayuz "model yukleme kac saniye surdu" bilgisini her
    soruda yeniden gostersebilir, model gercekten yeniden yuklenmeden.
    """

    def __init__(self, embed_model, embedding_client, chat_model, chat_client, load_seconds=0.0):
    ...
    def unload(self):
        """Her iki modeli de bellekten kaldirir."""
        self.embed_model.unload()
        self.chat_model.unload()


def load_models(app_name="local_rag", chat_progress_callback=None):
    """Embedding ve sohbet modellerini indirip belleğe yukler, kullanima
    hazir istemcilerle birlikte bir ModelBundle dondurur.

    app.py (CLI) ve arayuz.py (Streamlit) bu fonksiyonu ortak kullanir --
    model yukleme ve sohbet ayarlari (temperature, frequency_penalty,
    max_tokens) tek yerde tanimli kalir, iki arayuz de birbirinden
    bagimsiz olarak ayni ayarlarla calisir.
    """
    ...
    chat_model.download(chat_progress_callback)
    chat_model.load()
    ...
    return ModelBundle(
        embed_model, embedding_client, chat_model, chat_client,
        load_seconds=time.time() - load_start,
    )


class QueryResult:
    """answer_query()'nin dondurdugu, bir sorunun tum sonucunu tasiyan
    yapi. Hem app.py (CLI) hem arayuz.py (Streamlit) ayni alanlari
    kullanarak kendi arayuzlerine gore bicimlendirir.

    Alanlar:
        query: kullanicinin sordugu soru
        answer: kullaniciya gosterilecek nihai cevap metni
        target_region: tespit edilen kas grubu filtresi (tuple ya da None)
        target_equipment: tespit edilen ekipman filtresi (tuple ya da None)
        results: find_relevant'in dondurdugu (skor, parca) listesi --
            model_called=False durumlarinda (program istegi) bos olabilir
        elapsed_seconds: sorunun bastan sona cevaplanma suresi (saniye)
        model_called: sohbet modeli gercekten cagrildi mi (False ise
            cevap ya program istegi reddi ya da esik-alti "bilgi yok"tur)
        embedding_seconds: sorunun embedding vektorune cevrilme suresi
        search_seconds: find_relevant (filtreleme + kosinus benzerligi
            siralamasi) suresi
        generation_seconds: LLM'in cevabi uretme (streaming) suresi --
            model_called=False ise 0.0'dir, cunku model hic cagrilmadi
    """

    def __init__(self, query, answer, target_region, target_equipment,
                 results, elapsed_seconds, model_called,
                 embedding_seconds=0.0, search_seconds=0.0, generation_seconds=0.0):
        self.query = query
        self.answer = answer
        self.target_region = target_region
        self.target_equipment = target_equipment
...
def answer_query(query, chunks, embedding_client, chat_client):
    """Tek bir soruyu bastan sona cevaplar: niyet kontrolu (program
    istegi mi), retrieval, esik kontrolu, model cagrisi ve cevap
    temizligi. Hem app.py hem arayuz.py bu TEK fonksiyonu cagirir --
    RAG akisinin kendisi iki arayuz arasinda asla kopyalanmaz.

    QueryResult uzerinden asama bazli sureleri de dondurur (embedding,
    vektor arama, LLM uretimi) -- boylece arayuz.py bir sorunun nerede
    zaman harcadigini (orn. model yukleme mi, yoksa her seferinde
    tekrarlanan bir asama mi) ayri ayri gosterebilir.
    """
    start = time.time()

    if is_program_request(query):
        return QueryResult(
            query=query, answer=NO_INFO_MESSAGE, target_region=None,
            target_equipment=None, results=[],
            elapsed_seconds=time.time() - start, model_called=False,
        )
    ...
    embedding_seconds = time.time() - embed_start

    search_start = time.time()
    results = find_relevant(
        query_embedding, chunks,
        target_region=target_region, target_equipment=target_equipment,
    )
    search_seconds = time.time() - search_start

    # En iyi eslesme bile esigin altindaysa baglam alakasizdir -- modeli
    # hic cagirmadan direkt "bilgi yok" de.
    if not results or results[0][0] < SIMILARITY_THRESHOLD:
        return QueryResult(
            query=query, answer=NO_INFO_MESSAGE, target_region=target_region,
            target_equipment=target_equipment, results=results,
            elapsed_seconds=time.time() - start, model_called=False,
            embedding_seconds=embedding_seconds, search_seconds=search_seconds,
        )

    messages = [
        {"role": "system", "content": SYSTEM_PROMPT.format(context=build_context(results))},
    ...
    answer = clean_model_response(raw_response, results)

    return QueryResult(
        query=query, answer=answer, target_region=target_region,
        target_equipment=target_equipment, results=results,
        elapsed_seconds=time.time() - start, model_called=True,
        embedding_seconds=embedding_seconds, search_seconds=search_seconds,
